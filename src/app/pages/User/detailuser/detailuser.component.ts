...
export class DetailuserComponent implements OnInit, OnDestroy {
  ...
  loadUsers() {
    this.userService.FindById(this.id).subscribe((devs) => {
      this.userLoad = devs;
      this.datenaiss= this.datePipe.transform(this.userLoad.datenaissance,'dd-MM-yyyy');
      this.datecreation= this.datePipe.transform(this.userLoad.datecreation,'dd-MM-yyyy');
      this.photo= this.userLoad.photo;
      this.photo2='url('+this.userLoad.photo+')';
    });
  }
