...
@Component({
  selector: 'app-detailadoption',
  templateUrl: './detailadoption.component.html',
  styleUrls: ['./detailadoption.component.scss'],
...
export class DetailadoptionComponent implements OnInit, OnDestroy {
  @Input() id: number;
  isLoading$;
  customer: Customer;
  formGroup: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.loadCustomer();
    this.loadAdoptions();
    console.log(this.agence.quartier);
  ...
  loadAdoptions() {
    this.adoptionService.FindById(this.id).subscribe((devs) => {
      this.adoptionLoad = devs;
      this.datenaissanceorp = this.datePipe.transform(this.adoptionLoad.orphelin.datenaissance,'dd-MM-yyyy');
      this.datecreation = this.datePipe.transform(this.adoptionLoad.datecreation,'dd-MM-yyyy');
      this.dateinscrite = this.datePipe.transform(this.adoptionLoad.date,'dd-MM-yyyy');
      this.photo= this.adoptionLoad.orphelin.photo;
      this.photo2='url('+this.adoptionLoad.orphelin.photo+')';
    });
  }
  ...
  print(id:number): void {
    ...
    popupWin.document.write(`
      <html>
        <head>
          <title>Fiche infos adoption ${id}</title>
            ${linksHtml}
            ${stylesHtml}
            <style>
