...
@Component({
  selector: 'app-detailorphelin',
  templateUrl: './detailorphelin.component.html',
  styleUrls: ['./detailorphelin.component.scss'],
...
export class DetailorphelinComponent implements OnInit, OnDestroy {
  @Input() id: number;
  isLoading$;
  customer: Customer;
  formGroup: FormGroup;
  ...
  loadOrphelins() {
    this.orphelinService.FindById(this.id).subscribe((devs) => {
      ...
      this.datenaiss= this.datePipe.transform(this.orphelinLoad.datenaissance,'dd-MM-yyyy');
      this.dateentree= this.datePipe.transform(this.orphelinLoad.dateentree,'dd-MM-yyyy');
      this.photo= this.orphelinLoad.photo;
      this.photo2='url('+this.orphelinLoad.photo+')';
      console.log("sfdg"+this.photo);
    });
  }
  ...
  print(id:number): void {
    ...
    popupWin.document.write(`
      <html>
        <head>
          <title>Fiche infos orphelin ${id}</title>
            ${linksHtml}
            ${stylesHtml}
            <style>
