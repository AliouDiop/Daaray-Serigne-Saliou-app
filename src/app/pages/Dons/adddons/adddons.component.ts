...
import { DialogAdddonsComponent } from './dialog-adddons/dialog-adddons.component';
...
@Component({
  selector: 'app-adddons',
  templateUrl: './adddons.component.html',
  styleUrls: ['./adddons.component.scss']
})
export class AdddonsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  submit(param: any) {
    ...
    console.log("sfdgdg" + infodonateur);
    //Ajout orphelin
    this.donateurService.save(infodonateur)
      .subscribe((dona) => {
        if (dona) {
          console.log(dona)
          ...
          this.donsService.save(infoadons)
            .subscribe(data => {

              if (data) {
                console.log(" cool infoadoption ajout");
                const modalRef = this.modalService.open(DialogAdddonsComponent, { size: 'sm' });
  ...
  InitSubheader() {
    setTimeout(() => {
      this.subheader.setTitle('Ajout un don');
    }, 1);
  }
